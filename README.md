...
첫 기동 시 Flyway 가 데모 데이터를 자동으로 넣습니다 — **계정 3개 · 설문 18개(계정당 6개) · 응답 227건**.
로그인하면 목록·응답·대시보드(일별 추이, 문항별 분포, 평점 평균)가 이미 채워져 있습니다.

| 이메일 | 비밀번호 | 설문 구성 |
|---|---|---|
| `demo1@openforms.dev` | `demo1234!` | 사내 만족도·재택근무·구내식당 등 6개 |
| `demo2@openforms.dev` | `demo1234!` | 카페 신메뉴·배송·앱 사용성 등 6개 |
| `demo3@openforms.dev` | `demo1234!` | 스터디·컨퍼런스·부트캠프 등 6개 |

계정마다 **발행 4 · 종료 1 · 작성 중 1** 로 구성되어 상태별 화면을 모두 확인할 수 있습니다.
직접 만들어 보고 싶다면 **회원가입 → 폼 생성 → 발행 → 공개 링크로 응답 → 대시보드 확인** 순서로 진행하면 됩니다.

> 데모 데이터 없이 빈 DB 로 시작하려면 백엔드를 아래처럼 실행합니다. 이미 시드가 들어간 DB 라면
> `docker compose down -v` 로 초기화한 뒤 실행하십시오(적용된 마이그레이션이 사라지면 Flyway 검증에 걸립니다).
> ```bash
> SPRING_FLYWAY_LOCATIONS=classpath:db/migration ./gradlew bootRun
> ```
