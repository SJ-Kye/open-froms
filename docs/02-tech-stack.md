...
| 차트 | Recharts | v3 |
| HTTP 클라이언트 | axios | v1 |
| 아이콘 | lucide-react | v1.25 |
| 스타일 | CSS 변수(디자인 토큰) + CSS Modules | 별도 의존성 없음 |
...
- **스타일은 라이브러리 없이** CSS 변수로 만든 디자인 토큰 + CSS Modules 로 처리합니다. 색·타이포·모서리를 토큰 한 곳에 모으면 화면이 늘어나도 색을 고르는 자리가 하나로 유지되고, 화면 고유 스타일은 CSS Modules 가 스코프를 격리해 클래스명 충돌을 막습니다. 유틸리티 CSS 프레임워크를 얹을 만큼 화면 수가 많지 않아, 설정 파일과 의존성을 늘리는 대신 브라우저 기본 기능으로 같은 효과를 얻는 쪽을 택했습니다.
- **lucide-react**: 아이콘. 화면마다 인라인 SVG 를 늘리는 대신 tree-shaking 되는 아이콘 세트를 씁니다.
- **API 호출은 개발 서버 프록시**(`/api` → `localhost:8080`)를 지납니다. 브라우저 입장에서 동일 출처가 되어 **백엔드에 CORS 설정이 필요 없습니다**. 인증 헤더·쿠키 정책을 건드릴 일이 없다는 점에서 CORS 를 여는 것보다 표면이 작습니다.
