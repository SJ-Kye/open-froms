...
| POST | `/api/auth/login` | 로그인, 토큰 쌍 발급 | 🔓 | 200 | 400 / 401(자격 실패) |
| POST | `/api/auth/refresh` | 리프레시 토큰 회전, 토큰 쌍 재발급 | 🔓 | 200 | 400 / 401(토큰 무효) |
| POST | `/api/auth/logout` | 리프레시 토큰 폐기 | 🔓 | 204 | 400 |
| GET | `/api/auth/me` | 내 정보 조회 | 🔒 | 200 | 401 / 404(주체 없음) |

**액세스 토큰은 1시간, 리프레시 토큰은 14일**입니다. 액세스 토큰은 무상태 JWT 라 발급 후 서버가 폐기할 수 없어 수명을 짧게 두고, 대신 "갱신할 권리"를 DB 에 상태로 남기는 리프레시 토큰(불투명 난수)을 함께 발급합니다. 상태가 DB 에 있으므로 로그아웃·유출 대응 시 서버가 실제로 무효화할 수 있습니다.

**리프레시 토큰은 1회용입니다.** `/refresh` 는 제시된 토큰을 즉시 폐기하고 새 토큰 쌍을 내줍니다(회전). 액세스만 갱신하고 리프레시를 재사용하면 토큰 하나가 14일 내내 유효해져 회전의 의미가 사라지기 때문입니다. 회전에는 부수 효과가 하나 더 있습니다 — 유출된 사본과 정상 클라이언트가 같은 토큰을 쓰면 **나중 요청이 반드시 실패**하므로, 이미 폐기된 토큰이 다시 오는 것 자체가 유출 신호가 됩니다. 이때는 그 사용자의 리프레시 토큰을 **전부 폐기**하고 재로그인을 요구합니다. 반면 단순 만료는 정상적인 수명 종료이므로 다른 세션을 끊지 않습니다.

**`/refresh`·`/logout` 이 익명 허용인 이유**는 두 API 의 자격증명이 액세스 토큰이 아니라 **리프레시 토큰 자체**이기 때문입니다. 액세스 토큰 인증을 걸면 정작 그것이 만료된 뒤에는 갱신할 수 없어 목적이 무너집니다.

401 응답은 **없음·만료됨·이미 폐기됨을 구분하지 않고** 모두 `INVALID_REFRESH_TOKEN` 입니다(로그인의 `INVALID_CREDENTIALS` 와 같은 취지 — 확보한 토큰이 유효했던 것인지 확인할 수단을 주지 않습니다). 로그아웃도 같은 이유로 **멱등**입니다. 이미 없거나 무효한 토큰이어도 204 로 답합니다. 목적하는 상태("이 토큰은 더 이상 쓰이지 않는다")가 이미 달성되어 있고, 실패로 답하면 토큰의 존재 여부를 알려 주게 됩니다. 액세스 토큰은 무상태라 로그아웃 후에도 만료 전까지 유효하므로 **클라이언트가 함께 폐기해야 합니다.**
