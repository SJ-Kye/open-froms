...
erDiagram
    ...
    refresh_tokens {
        ...
        varchar token_hash UK "NOT NULL (SHA-256 hex · 원문 미보관)"
        timestamp issued_at "NOT NULL"
        timestamp expires_at "NOT NULL"
        timestamp revoked_at "NULL 허용 (회전·로그아웃·유출 탐지)"
    }
    forms {
        bigint id PK
        uuid user_id FK "NOT NULL"
...
### refresh_tokens — 액세스 토큰 갱신 자격증명
| 컬럼 | 타입 | 제약 | 설명 |
|---|---|---|---|
| id | BIGINT | PK, IDENTITY | |
| user_id | UUID | NOT NULL, FK→users(id) ON DELETE CASCADE | 토큰의 주인 |
| token_hash | VARCHAR(64) | NOT NULL, UNIQUE | 토큰 원문의 SHA-256 hex. **원문은 저장하지 않습니다** |
| issued_at | TIMESTAMP | NOT NULL | 발급 시각 |
| expires_at | TIMESTAMP | NOT NULL | 만료 시각 (발급 + 14일) |
| revoked_at | TIMESTAMP | NULL | 폐기 시각. 회전·로그아웃·유출 탐지로 채워지며, 유효하면 NULL |

**이 테이블이 존재하는 이유는 액세스 토큰이 무상태이기 때문입니다.** JWT 는 서명만 맞으면 만료 전까지 서버가 폐기할 수 없어, 수명을 짧게(1시간) 두는 것 외에 통제 수단이 없습니다. 그래서 "갱신할 권리"만 DB 에 상태로 남겨, 로그아웃과 유출 대응 시 서버가 실제로 무효화할 수 있게 했습니다. 인증 자체는 그대로 무상태이며(요청마다 DB 를 보지 않습니다), 상태를 갖는 것은 갱신 경로뿐입니다.

**원문 대신 해시를 저장합니다.** DB 가 유출되어도 그 값으로 갱신할 수 없어야 하기 때문입니다. 비밀번호와 달리 BCrypt 를 쓰지 않은 것은 **해시로 조회**해야 하기 때문입니다 — BCrypt 는 행마다 salt 가 달라 동등 비교가 불가능해 전 행을 훑어야 합니다. 원문이 256비트 난수(`SecureRandom`)라 사전 공격의 대상이 아니므로 key stretching 이 주는 이득도 없습니다.

**사용한 토큰을 지우지 않고 `revoked_at` 으로 남기는 것이 이 설계의 핵심입니다.** 리프레시 토큰은 1회용이라 갱신할 때마다 회전(기존 폐기 + 신규 발급)되는데, 행을 삭제해 버리면 "회전된 토큰"과 "존재한 적 없는 토큰"을 구분할 수 없습니다. 폐기 흔적이 남아 있어야 **이미 쓴 토큰이 다시 오는 것**을 유출 신호로 읽고 그 사용자의 토큰을 전량 폐기할 수 있습니다.

`user_id` 인덱스는 그 전량 폐기를 위한 것입니다. 토큰 단건 조회는 `token_hash` 의 유니크 인덱스가 이미 커버하므로, 별도 인덱스가 필요한 조회 패턴은 "한 사용자의 토큰 전부"뿐입니다.

**감사 컬럼은 두지 않습니다.** 생성 주체가 언제나 토큰 소유자 본인이고 시각은 `issued_at`·`revoked_at` 이 이미 담고 있어, 감사 4종이 새로운 정보를 더하지 않습니다(아래 "감사 컬럼 정책"의 기준과 같습니다).
...
### api_call_logs — API 호출 이력 (인프라 테이블)
| 컬럼 | 타입 | 제약 | 설명 |
|---|---|---|---|
| id | BIGINT | PK, IDENTITY | |
| trace_id | VARCHAR(64) | NOT NULL | 요청 1건의 추적 ID. 응답 헤더로도 나가는 값과 동일합니다 |
| method | VARCHAR(10) | NOT NULL | HTTP 메서드 |
| path | VARCHAR(500) | NOT NULL | 요청 경로 |
| status | INT | NOT NULL | 응답 상태 코드 |
| principal | VARCHAR(100) | NULL | 호출 주체(이메일). 비인증 요청이면 NULL |
| duration_ms | BIGINT | NOT NULL | 처리 소요 시간(ms) |
| created_at | TIMESTAMP | NOT NULL | 호출 시각 |

**이 테이블만 도메인이 아니라 운영 로그입니다.** 위 ERD 의 테이블들과 달리 어떤 도메인 개념도 표현하지 않고, FK 도 하나 없으며, 지워도 서비스 동작이 달라지지 않습니다. 그래서 ERD 에 넣지 않고 여기 별도 항목으로 둡니다. 적재는 `ApiCallLogInterceptor` 가 요청 종료 시점에 메타데이터를 모아 `ApiCallLogWriter` 로 넘기고, 쓰기는 비동기로 이루어져 응답 지연에 얹히지 않습니다.

**메타데이터만 저장하고 본문은 저장하지 않습니다.** 요청/응답 body 를 남기면 비밀번호와 JWT 가 그대로 로그 테이블에 복사되어, 계정 저장소를 BCrypt 로 보호한 의미가 사라집니다. 무엇이 얼마나 걸려 어떤 상태로 끝났는지는 위 7컬럼으로 충분히 답할 수 있어, 본문을 남길 이유가 없습니다.

**주체를 `user_id` FK 가 아닌 문자열로 둔 것도 같은 이유(독립성)입니다.** FK 로 두면 매 요청마다 사용자 조회가 필요하고, 인프라 로그가 도메인 테이블의 생명주기에 묶여 계정 삭제가 로그 보존과 충돌합니다. 값의 출처는 감사 컬럼의 `created_by` 와 동일한 인증 주체이며, 다만 비인증 요청을 `ANONYMOUS` 가 아니라 NULL 로 둡니다 — 감사 컬럼에서는 "익명 제출"이 도메인 사실이지만 여기서는 "주체 없음"이 사실 그대로이기 때문입니다.

**감사 컬럼을 상속하지 않고 `created_at` 하나만 둡니다.** 이 행은 생성 이후 수정되지 않고, 주체는 `principal` 이 이미 담고 있습니다(아래 "감사 컬럼 정책"의 기준과 같습니다).
...
| refresh_tokens | X | 생성 주체가 항상 토큰 소유자 본인이고, 시각은 `issued_at`·`revoked_at` 이 이미 담습니다 |
| api_call_logs | X (자체 `created_at`) | 도메인 엔티티가 아닌 운영 로그이며 불변입니다. 주체는 `principal` 이 이미 담습니다 |
...
| `uk_refresh_tokens_token_hash` | refresh_tokens(token_hash) | UNIQUE | 갱신 요청마다 해시로 토큰 단건 조회 + 중복 발급 방지 |
| `ix_refresh_tokens_user_id` | refresh_tokens(user_id) | 일반 | 유출 탐지·로그아웃 시 한 사용자의 토큰 전량 폐기 (FK) |
| `ix_api_call_logs_trace_id` | api_call_logs(trace_id) | 일반 | traceId 로 한 요청의 이력을 추적 |
| `ix_api_call_logs_created_at` | api_call_logs(created_at) | 일반 | 기간별 호출 이력 조회 |
...
## 제약조건 요약
- **NOT NULL**: 모든 식별/상태/시각/감사 컬럼에 적용합니다. NULL 을 허용하는 것은 응답값(`option_id`·`text_value`·`number_value`·`date_value`·`time_value` — 타입에 따라 하나만 채워집니다), 질문 범위 메타(`min_value`·`max_value`), `forms.description`, 아직 일어나지 않은 사건(`forms.published_at`·`forms.closed_at`·`refresh_tokens.revoked_at`), 그리고 주체가 없을 수 있는 `api_call_logs.principal` 뿐입니다.
...
- **UNIQUE**: users.email, forms.slug, refresh_tokens.token_hash 에 적용합니다.
...
- 적용된 스크립트는 `V1__init.sql`(초기 스키마), `V2__api_call_logs.sql`(API 호출 이력), `V3__form_lifecycle_timestamps.sql`(`forms.published_at`·`closed_at`), `V4__refresh_tokens.sql`(리프레시 토큰)입니다.
